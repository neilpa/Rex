...
public func <~<Control: BindingValueConsumer, Value, Error where Control.Value == Value, Control.Error == Error>(control: Control, producer: SignalProducer<Value, Error>) {
    control.bindToProducer(producer.map { BindingValue<Value>.Value($0) })
}

public func <~<Control: BindingValueConsumer, Value, Error where Control.Value == Value, Control.Error == Error>(control: Control, signal: Signal<Value, Error>) {
...
public func <~<Control: BindingValueConsumer, Value: Equatable, Error where Control.Value == Value, Control.Error == Error>(control: Control, producer: SignalProducer<[Value], Error>) {
...
public func <~<Control: BindingValueConsumer, Value, Error where Control.Value == Value, Control.Error == Error>(control: Control, producer: SignalProducer<BindingValue<Value>, Error>) {
